"""Open /badges in a VISIBLE Chrome window with half the badges lit (some gold frames), and keep it open."""
...
PROFILE = os.path.join(HERE, "chrome_profile_live")
...
res = cdp.send("Runtime.evaluate", {"expression": r"""
  (() => {
    ...
    return JSON.stringify({frameImgs: counts, litFrames: frames.length});
  })()
""", "returnByValue": True})
print("DOM stats:", res.get("result", {}).get("value"))
print("浏览器窗口已打开 http://localhost:3000/badges，保持 10 分钟。查看后可手动关闭窗口。")
time.sleep(600)
print("done")
