"""Verify enlarged cabinet badges (66px circle, 20px name) — lips, group bottoms, overflow, overlap."""
...
PROFILE = os.path.join(HERE, "chrome_profile_big")
...
(() => {
  ...
  info.cabinet = cRect;
  // shelf rows: direct children positioned absolute (computed style)
  ...
  info.rows = rows.map((row, i) => {
    ...
    const badges = groups.map(b => {
      ...
      const desc = b.querySelectorAll('p')[1];
      const status = b.querySelector('span');
      const sR = status ? status.getBoundingClientRect() : null;
      return {
        boxLeft: Math.round(br.left),
        boxBottom: Math.round(br.top + br.height),
        circle: cR ? {top: Math.round(cR.top), bottom: Math.round(cR.top + cR.height), w: Math.round(cR.width), cy: Math.round(cR.top + cR.height/2)} : null,
        img: iR ? {w: Math.round(iR.width)} : null,
        name: name ? name.textContent.trim() + '|' + Math.round(name.getBoundingClientRect().width) + 'px' : null,
        desc: desc ? desc.textContent.trim() : null,
        status: status ? status.textContent.trim() : null,
        statusCut: sR ? (sR.bottom > br.bottom ? 'belowGroup' : 'ok') : null,
        earned: (circle.className||'').includes('FFE896'),
      };
    });
    const scrollEl = row.querySelector('div.flex-1');
    const scrollW = scrollEl ? scrollEl.scrollWidth : null;
    const clientW = scrollEl ? scrollEl.clientWidth : null;
    const arrow = row.querySelector('button');
    return {idx: i, rowTop: Math.round(rRect.top - cabinet.getBoundingClientRect().top),
            rowTopPct: row.style.top, groupBottom: badges.length ? Math.max(...badges.map(b => b.boxBottom)) : null,
            badges, overflowX: scrollW > clientW, arrow: !!arrow};
  });
  // arrow count across rows
  info.arrowCount = info.rows.filter(r => r.arrow).length;
  // recentCard + book positions
  const recentTag = [...document.querySelectorAll('div')].find(e => e.textContent.trim() === '最近获得');
  if (recentTag) {
    let rc = recentTag;
    while (rc && getComputedStyle(rc).position !== 'absolute') rc = rc.parentElement;
    info.recentCard = rc ? clamp(rc) : null;
  }
  ...
  if (shareBtn) {
    ...
    for (let i=0;i<8;i++) { bookRoot = bookRoot.parentElement; const r = bookRoot.getBoundingClientRect(); if (r.width > 500) { info.bookBlock = clamp(bookRoot); break; } }
  }
  // cabinet vs book overlap
  if (info.bookBlock) {
    const cB = cabinet.getBoundingClientRect().top + cabinet.getBoundingClientRect().height;
    info.gapCabinetBook = Math.round(info.bookBlock.top - cB);
  }
