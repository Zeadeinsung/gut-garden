"""Measure the 分享我的徽章墙 button position relative to the badge book."""
...
PROFILE = os.path.join(HERE, "chrome_profile_share")
...
shot(cdp, "share_btn.png")

res = cdp.send("Runtime.evaluate", {"expression": r"""
(() => {
  const clamp = (el) => { const r = el.getBoundingClientRect(); return {left: Math.round(r.left), top: Math.round(r.top), w: Math.round(r.width), h: Math.round(r.height), right: Math.round(r.right), bottom: Math.round(r.bottom)}; };
  const shareBtn = Array.from(document.querySelectorAll('button')).find(b => (b.textContent||'').replace(/\s+/g,'').includes('分享我的徽章墙'));
  if (!shareBtn) return JSON.stringify({err: 'no share btn'});
  const btn = clamp(shareBtn);
  // find book block: nearest ancestor with width > 500
  let book = shareBtn;
  for (let i=0;i<10;i++) {
    book = book.parentElement;
    const r = book.getBoundingClientRect();
    if (r.width > 500) { return JSON.stringify({btn, book: clamp(book), offsetBtnRightFromBookRight: Math.round(btn.right - r.right)}); }
  }
  return JSON.stringify({err: 'no book ancestor'});
})()
""", "returnByValue": True})
