"""Screenshot /badges full page to verify the three-card restyle."""
...
PROFILE = os.path.join(HERE, "chrome_profile_recheck")
...
try:
    ...
    with open(os.path.join(HERE, "badges_recheck_full.png"), "wb") as f:
        f.write(base64.b64decode(res["data"]))
    print("saved badges_recheck_full.png")

    # viewport-only capture
    send("Emulation.setDeviceMetricsOverride", {"width": 1400, "height": 1000, "deviceScaleFactor": 1, "mobile": False})
    time.sleep(1)
    res = send("Page.captureScreenshot", {"format": "png"})
    with open(os.path.join(HERE, "badges_recheck_view.png"), "wb") as f:
        f.write(base64.b64decode(res["data"]))
    print("saved badges_recheck_view.png")
