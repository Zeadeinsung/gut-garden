...
PROFILE = os.path.join(HERE, "chrome_profile_checkin_dash")
...
def scroll_by(cdp, dy):
    cdp.send("Runtime.evaluate", {"expression": r"""
      (() => { const el = document.querySelector('.flex-1.overflow-auto') || document.scrollingElement;
        el.scrollTop += %d; return el.scrollTop; })()
...
def main():
    ...
    wait_load(cdp, "http://localhost:3000" + route, w=w, h=h)
    m = metrics(cdp)
    print("metrics:", json.dumps(m, ensure_ascii=False))
    shot(cdp, f"checkin_dash_{tag}_top.png")
    cdp.ws.close()
    kill_port(PORT)
    print("done")
