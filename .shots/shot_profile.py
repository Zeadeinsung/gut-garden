...
PROFILE = os.path.join(HERE, "chrome_profile_header")
...
(() => {
  ...
  ['gg-block-positions-home', 'gg-block-positions-profile'].forEach(del);
})();
"""
...
def main():
    ...
    wait_load(cdp, "http://localhost:5173/profile")
    shot(cdp, "profile_full.png")
    cdp.ws.close()
    kill_port(PORT)
    print("done")
