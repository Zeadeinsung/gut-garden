...
COMMENT ON TABLE friendships IS '好友关系（儿童之间，双向各存一行或成对存储）';

CREATE UNIQUE INDEX uk_friendship_pair ON friendships(child_id, friend_child_id);
CREATE INDEX idx_friendship_child ON friendships(child_id);
